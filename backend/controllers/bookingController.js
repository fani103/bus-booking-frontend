...
const sendBookingEmail = async (booking, pdfBuffer) => {
  if (!booking.email) {
    ...
    <div style="font-family:Arial,sans-serif;background:#f4f7fb;padding:20px;">
      <div style="max-width:600px;margin:auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 6px 20px rgba(0,0,0,0.1);">
        ...
        <div style="padding:22px;">
          ...
          <p style="margin-top:24px;">Your PDF ticket is attached with this email.</p>
          <p>Thank you for choosing <b>Fun Travels</b> ❤️</p>
        </div>
  ...
  await transporter.sendMail({
    ...
    attachments: [
      {
        filename: `Fun-Travels-${booking.bookingId}.pdf`,
        content: pdfBuffer,
  ...
  console.log("Email + PDF sent ✅ to:", booking.email);
};
...
`;
    ...
    const pdfBuffer = await generateTicketPDF(newBooking, qrImage);

    try {
      await sendBookingEmail(newBooking, pdfBuffer);
    } catch (emailError) {
